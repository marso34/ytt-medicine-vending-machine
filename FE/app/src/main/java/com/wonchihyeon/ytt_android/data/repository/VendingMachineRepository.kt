...
class VendingMachineRepository(private val apiService: ApiService) {
    ...
    fun getNearByMachine(latitude: Double, longitude: Double, callback: (ResponseDTO<List<VendingMachineDTO>>?) -> Unit) {
        apiService.getNearByMachine(latitude, longitude).enqueue(object : Callback<ResponseDTO<List<VendingMachineDTO>>> {
